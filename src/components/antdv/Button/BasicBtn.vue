<template>
  ...
  >
    <template #icon>
      <Icon v-if="icon" :icon="icon" :size="16" :color="iconColor" />
    </template>
...
<script setup name="CBtn">
  ...
  const whiteIconBtn = ['查询', '新增', '关闭'];
  const iconColor = computed(() => {
    return whiteIconBtn.indexOf(props.showType) > -1 ? '#fff' : '';
  });
  ...
  const attrsMap = new Map([
    ...
    [BTN_TYPE.bindingUrl, { type: 'text' }],
  ]);

  const styleMap = new Map([
    [BTN_TYPE.search, { 'box-shadow': '0 2px 0 rgba(5, 111, 255, 0.18)' }],
    [BTN_TYPE.reset, {}],
    [BTN_TYPE.add, { type: 'primary', 'box-shadow': '0 2px 0 rgba(5, 111, 255, 0.18)' }],
    ...
    [BTN_TYPE.close, { 'box-shadow': '0 2px 0 rgba(5, 111, 255, 0.18)' }],
    [BTN_TYPE.bindingUrl, { color: 'var(--custom-primary-color)' }],
  ]);
