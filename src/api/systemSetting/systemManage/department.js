...
export const officeListApi = ({ current, size, officeName }) => {
  return axiosInstance.get(Api.OFFICELIST + `?current=${current}&size=${size}`, {
    officeName,
  });
};
...
export const officeTreeApi = ({ officeName, name }) => {
  let param = {};
  if (officeName || name) {
    param = {
      params: {
