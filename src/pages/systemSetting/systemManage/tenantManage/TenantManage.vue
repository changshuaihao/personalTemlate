<template>
  <div>
    ...
    <a-modal v-model:open="showModel" width="650px" :title="showEdit ? '编辑租户' : '新增租户'">
      <template #footer>
    ...
    <a-modal v-model:open="showTemplateModel" width="1000px" title="自定义配置计算模板">
      <a-table
...
<script setup>
  ...
  const confiMenu = async () => {
    ...
    if (state.selectedRowKeys.length === 1) {
      ...
      const { data: menu } = await menuTreeApi({ roleId: '', tenantId: state.selectedRowKeys[0] });
      let idList = [];
      if (menu.data) {
        const getIds = (data) => {
          data.forEach((item) => {
